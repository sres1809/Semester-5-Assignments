...
def create_packet(frame_number, message):
    ack = "0101010" + "+" + "Sndr_2" + multiprocessing.current_process().name + message + frame_number
    return ack


def receiver2(conn, packets_to_channel):
    #print("In receiver 2")
    char_list = []
    while 1:
        packet_received = conn.recv()
        #print("Packet received at receiver 2!")
        if len(packet_received) == 24:
            break
        codeword = decode_frame(packet_received)
        correct = check_CRC(codeword[:32])
        if correct == False:
            print("Error detected in receiver 2")
            packet = create_packet(codeword[-1], "ERR")
            packets_to_channel.put(packet)
            #print("Error message sent from receiver 2")
        else:
            #print("No error detected in receiver 2")
            packet = create_packet(codeword[-1], "Ack")
            packets_to_channel.put(packet)
            print("Acknowledgement sent from receiver 2")
            data = decode_codeword(codeword[:28])
            if data == "END!":
                break
            char_list.append(data+codeword[32])

    print("Receiver 2 complete")
    char_list = list(set(char_list))
    char_list.sort(key=frame_number)
    line = []
    for chars in char_list:
        line.append(chars[:4])
    file = open("ReceiverFile2.txt", 'w')
