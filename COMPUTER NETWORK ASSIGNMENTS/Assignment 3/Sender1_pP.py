...
def sender1(conn, packets_to_channel):
    ...
    while len(packet_sent) != 0 or ready_queue.empty() == False:
        ...
        while((packets_to_channel.empty()==False)):
            print("Attempt to acquire channel by sender 1 failed!")
        ...
        if p > 1:
            ...
            print("Channel not acquired by sender 1!")
            continue

        print("Sender 1 acquires the channel and sends packet!")

        if conn.poll(timeout):
            packet_received = conn.recv()
        else:
            print("Timeout elapsed in sender 1!")
            continue
